...
dotnet restore (Join-Path $root 'Pomodoro.sln')
...
& $compiler (Join-Path $root 'installer\Pomodoro.iss')
if ($LASTEXITCODE -ne 0) { throw 'Fallo al generar el instalador.' }

$setup = Join-Path $dist 'Pomodoro-Setup.exe'
$hash = (Get-FileHash -Algorithm SHA256 $setup).Hash.ToLowerInvariant()
"$hash  Pomodoro-Setup.exe" | Set-Content -Encoding ascii (Join-Path $dist 'Pomodoro-Setup.exe.sha256')
